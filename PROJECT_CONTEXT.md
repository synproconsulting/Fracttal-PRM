...
- **Rate limiting (Sprint 25 PR B / AD-44; keying fixed in the Sprint 25 hotfix / FPRM-460 / AD-46):** `POST /auth/login`, `POST /auth/password-reset/request|confirm`, and the public draft-token application endpoints (`POST /applications`, `PATCH /applications/{id}`, `POST /applications/{id}/submit`, `POST /applications/{id}/documents`) return **429** once the per-IP limit is exceeded (env-tunable: `RATE_LIMIT_LOGIN` 10/min, `RATE_LIMIT_PASSWORD_RESET` 5/min, `RATE_LIMIT_PUBLIC_APP` 20/min). Authenticated app traffic is not limited. The limiter keys on the **real client IP behind Railway's edge proxy** via `rate_limiter.get_client_ip` (`X-Envoy-External-Address`, then left-most `X-Forwarded-For`, else TCP peer) — `get_remote_address` alone keyed on the rotating proxy peer so the control was inert in prod (FPRM-460). A limited (429) response now carries `Retry-After` + `X-RateLimit-*` (`headers_enabled=True`); the 8 limited endpoints declare a `response: Response` param so slowapi can inject them.
...
| GET | `/internal/deals` | channel_manager / channel_ops_admin / system_admin | Sprint 8 / FPRM-134. Internal queue. Supports `?status=&partner_org_id=&limit=&offset=`. Orders by `submitted_at` desc nulls-last. partner_admin → 403. `from_date` / `to_date` filters applied to `submitted_at` (PR #178 fix). Same pattern as `GET /deal-registrations` (PR #175) and `GET /partners/{id}/pipeline`. End-of-day handling on `to_date`. 422 on malformed date strings. |
...
| GET | `/partners/{id}/dashboard/summary` | system_admin / channel_ops_admin / channel_manager / partner_admin (own) / partner_user (own) | Sprint 11 / FPRM-183 (widened in Sprint 12 / FPRM-190; partner_user own-org added Sprint 26 PR B / FPRM-461, mirroring FPRM-458). Partner home roll-up: deals counts by status, activation `items_complete/items_total`, document counts. Own-org enforced in the handler (AD-9); cross-org partner access 403s. |
...
| GET | `/internal/partners` | system_admin / channel_ops_admin / channel_manager | Sprint 12 / FPRM-205. Cross-org partner list with search/status/tier/category filters, page/page_size pagination, activation status join. **Sprint 26 PR B / FPRM-465:** each item also carries a nullable `channel_manager_name` = the first-assigned CM (earliest `partner_channel_managers.assigned_at`, tie-break lowest `id`, joined to `users.full_name`), resolved in one batched query (no N+1); sortable via `sort_by=channel_manager_name` (correlated subquery, unassigned sorts last in both directions); the CSV export gains a **Channel Manager** column. `enforce_cm_scope` is NOT applied — roster read, not in the AD-45 `CM_SCOPED_ACTIONS` set. |
...
| GET | `/partners/{id}/pipeline` | partner_admin / partner_user (own org only) | Sprint 14 / FPRM-229; **Sprint 25 hotfix / FPRM-458** widened from partner_admin-only to also allow `partner_user` for their own org (same own-org tenant scoping; amends FPRM-229). Deals grouped by status into 6 keys (`draft`/`submitted`/`under_review`/`info_required`/`approved`/`rejected`). Optional `status`/`from_date`/`to_date` filters. 403 to internal roles by design — internal users use `/internal/reports/pipeline`. Cross-org still 403 (tenant-isolation sweep stays green). `from_date` / `to_date` applied to `submitted_at` (PR #175 fix). End-of-day handling on `to_date`. 422 on malformed date strings. |
...
| GET | `/internal/quotes` | channel_manager / channel_ops_admin / system_admin | Sprint 18 / FPRM-287. Cross-deal quote dashboard. Filters: `status` / `partner_org_id` / `feature_plan` / `search` (matches `quote_name` or `deal_name`). Pagination: `page` (>=1) / `page_size` (1–100, default 20). Returns `{items, total, page, page_size, summary}`. `summary` rolls up system-wide totals and pipeline value (expired excluded). Joins Quote → active QuoteVersion → DealRegistration → PartnerOrganization. **Sprint 21 hotfix (FPRM-357):** each item now includes `deal_status` from the joined `DealRegistration.status` so the InternalQuotes dashboard can render a tinted Deal Status column. |
| GET | `/partners/{partner_org_id}/quotes` | partner_admin / partner_user (own org only) | Sprint 18 / FPRM-291. Partner-facing quote history scoped to the user's own org. Internal users are 403'd (use `/internal/quotes`). Optional `?status=` filter. Returns `{items: [{quote_name, deal_name, deal_status, feature_plan, currency_code, grand_total_after_discount, status, active_version, active_scenario, ...}]}` with grand totals joined from the active non-deleted version. `deal_status` added in Sprint 21 (closes the PR #172 TODO); PortalQuotes.jsx renders it as a tinted Deal Status column. |
...
└── pages/
    ...
    ├── PartnerProfile.jsx           # Sprint 7 / FPRM-106 → restyled Sprint 8 / FPRM-122 — adapts to /portal/profile and /internal/partners/:id/profile. Floating-label edit form, save button top-right, completeness progress bar + badge. Sprint 25 hotfix / FPRM-459 — OrgSummary shows a read-only "Partner System ID" (org id) in INTERNAL mode only (showSystemId={internalMode}); never in portal mode, excluded from edit + PATCH.
...
**Decision:** `partner_documents` stores the binary as base64 `file_data` (Text, nullable) following the AD-19 pattern, alongside the existing `partner_org_id` (FK, NOT NULL), `uploaded_by_user_id` (FK → users.id), `document_type` (String — free-form, not enum-constrained), `document_name`, `file_size_bytes`, `mime_type`, `uploaded_at`, and the review/approval fields (`status`, `review_notes`, `reviewed_by_user_id`, `reviewed_at`, `expiry_date`). `file_path` is retained but relaxed to nullable so legacy rows continue to validate.

Cross-record links are stored in `document_references`: `(id, document_id, entity_type, entity_id, label, created_at)`. `entity_type` is a polymorphic discriminator (`quote`, `quote_version`, `deal`, etc.); `entity_id` is the target UUID; `label` carries the semantic tag (e.g. `quote_acceptance`, `purchase_order`). No FK on `(entity_type, entity_id)` — a union-typed FK would require non-portable triggers. Two indexes: `(entity_type, entity_id)` for "what's attached to this quote/deal" and `(document_id)` for "where is this doc used".

The legacy `quote_documents` table (migration 033) is retired in Sprint 21 / migration 036: existing rows are backfilled into `partner_documents` (status `approved`) + `document_references` (`entity_type='quote'`, `label` = the original `document_type`), then the table is dropped.

**Tenant isolation (SOC II / ISO 27001 boundary):** `partner_org_id` on `partner_documents` is the hard isolation boundary. Every endpoint — read, write, download, delete — enforces `document.partner_org_id == partner_id` from the route plus, for partner roles, `current_user.partner_org_id == partner_id`. The `_load_doc_or_404` helper in `documents_router.py` collapses both checks into one place. Document references are loaded only through a document that has already passed the tenant check — they never expose cross-partner data. Every document read, write, download, and delete emits an audit log event.
...
**Consequence:** The quote acceptance gate (originally on `quote_documents`) now joins `document_references` → `partner_documents`: `PATCH /quotes/{id}/status` with `status=accepted` requires a `document_references` row with `entity_type='quote'`, `entity_id=<quote_id>`, `label='quote_acceptance'` linked to any non-deleted `partner_documents` row. The Sprint 21 hotfix (FPRM-353) explicitly dropped the `status='approved'` requirement on the partner document — the `status` field is for the KYC review workflow, not quote acceptance; the attachment via `document_references` is the affirmative evidence act. Uploading a document for a quote is a two-step operation: `POST /partners/{partner_org_id}/documents` (creates the file), then `POST /partners/{partner_org_id}/documents/{doc_id}/references` (links it to the quote). The `QuoteDetail.jsx` document section drives the upload+reference flow AND offers a Pick Existing tab that creates a reference-only against any existing partner document (`GET /partners/{id}/documents`).

**Do not:** Create any new table that stores file content (`file_data` or `file_path`) outside of `partner_documents` / `document_versions`. Do not access `document_references` rows without first verifying the parent `partner_documents.partner_org_id` against the current user's org. Do not return `file_data` in any list / metadata / patch response — the centralised allow-list in `documents_router.py` enforces this; the download endpoint is the only path that serves raw bytes. Do not add `document_type` as a Postgres ENUM — the free-string column accommodates both KYC types and transaction evidence types in one table without a migration per new type.

---

### AD-34 — Document binary storage moved to `document_versions`; `partner_documents.file_data` deprecated

**Decision:** From Sprint 22 onwards every uploaded byte goes into `document_versions.file_data`. A document has one row in `partner_documents` (metadata + the SOC II / ISO 27001 tenant boundary on `partner_org_id`) and one or more rows in `document_versions` (binary content, exactly one with `is_current=true`). `partner_documents.current_version_number` and `version_count` are denormalised pointers maintained atomically with version inserts.

`partner_documents.file_data` is retained on the table because dropping it would require a destructive migration on existing rows, but new code must never read or write it. The deprecation is signalled in `models.py` with an explicit comment and in `documents_router.py` by routing every reader through `DocumentVersion where is_current=true`.

**Tenant isolation:** unchanged — `partner_org_id` on `partner_documents` is still the boundary. `document_versions` inherits via the FK CASCADE on `document_id`; the existing `_load_doc_or_404` helper handles the check before any version endpoint touches `document_versions`.

**Why:** Sprint 21 stored every upload as a single immutable blob in `partner_documents.file_data`. "Upload a new version" required creating a whole new document with no link to the prior, and `document_references` rows on the prior would silently survive on a now-orphaned older file. Versioning at the binary layer keeps the metadata row + every cross-record link stable while the bytes evolve. The denormalised pointers on `partner_documents` mean list views never join `document_versions` for their primary read.

**Consequence:** Download / preview / per-version download all resolve via `document_versions where is_current=true`; the upload endpoint creates v1 on the new path; `POST .../versions` increments; revert flips `is_current` without deleting history. The acceptance gate in `quotes_router.py` now consults `document_type_rules.requires_approval` (AD-33 corollary): the seed row for `quote_acceptance` has `requires_approval=false`, preserving the FPRM-353 hotfix behaviour; admins flipping the rule to `true` reinstates the `PartnerDocument.status='approved'` check without code changes.

**Hotfix (FPRM-383/384/385):** Three behaviours were refined just after Sprint 22 closed. (1) Partner-admin self-service `DELETE /partners/{id}/documents/{doc_id}` now *permanently* deletes an unreferenced document (`db.delete` — `document_versions` cascade via FK `ondelete=CASCADE` + ORM `cascade="all, delete-orphan"`) instead of soft-flagging `status='rejected'`; the 409 path when references exist is unchanged. (2) Upload `initial_status` derivation: `auto_approve=true` rule → `approved`; `requires_approval=true` rule → `pending_review`; **no matching rule → `approved`** (default auto-approve) — previously a missing rule defaulted to `pending_review`. (3) `DELETE /admin/document-type-rules/{id}` no longer 409s when documents of the type exist — rules are freely deletable; existing documents keep their current status (no cascade).
...
### AD-46 — Rate-limit keying is proxy-aware; runtime security controls are verified live, not only in CI (Sprint 25 hotfix / FPRM-460)

**Decision:** The shared `slowapi` limiter (`backend/rate_limiter.py`) keys on the real external client IP behind Railway's edge proxy via `get_client_ip`: prefer `X-Envoy-External-Address` (Envoy-set, overwritten each hop → not client-forgeable), then the left-most `X-Forwarded-For` hop, each validated as a real IP, falling back to `get_remote_address` (TCP peer) for direct/local/test hits. The limiter is built with `headers_enabled=True`, so a limited response carries `Retry-After` + `X-RateLimit-*`; this requires every `@limiter.limit`-decorated endpoint to declare a `response: Response` parameter (slowapi injects into it), which was added to the 8 limited endpoints (login, register, password-reset request/confirm, the 4 public draft-token application endpoints).

**Why:** The AD-44 limits used `get_remote_address` (`request.client.host`). Behind Railway's proxy that peer is the proxy, and Railway routes successive requests from rotating internal source addresses, so every request landed in a distinct per-IP bucket and the counter never accumulated — 30 rapid logins returned all 401, zero 429, no rate-limit headers in prod, while CI was green because `TestClient` keeps one stable peer. The control was inert exactly where it mattered.
