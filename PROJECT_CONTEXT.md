...
**Consequence:** The quote acceptance gate (originally on `quote_documents`) now joins `document_references` → `partner_documents`: `PATCH /quotes/{id}/status` with `status=accepted` checks for a `document_references` row with `entity_type='quote'`, `entity_id=<quote_id>`, `label='quote_acceptance'` that resolves to a `partner_documents` row with `status=approved`. Uploading a document for a quote is a two-step operation: `POST /partners/{partner_org_id}/documents` (creates the file), then `POST /partners/{partner_org_id}/documents/{doc_id}/references` (links it to the quote). The `QuoteDetail.jsx` document section drives both steps; a future picker pass can surface existing approved partner documents as attach candidates without re-upload (`GET /partners/{id}/documents?status=approved`).
