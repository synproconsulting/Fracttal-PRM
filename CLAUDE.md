...
**Started:** May 2026
**Current state:** **Phase 5 complete (Sprints 15–18).** Sprint 18 closed the quoting module with scenario management, multi-currency display, and the internal quote dashboard. **Quote scenario management (FPRM-283 / AD-24)**: new endpoints `PATCH /quotes/{id}/active-scenario` and `GET /quotes/{id}/scenarios` group non-deleted versions by `scenario_label` and return the latest version per label in canonical good/better/best order. `QuoteDetail.jsx` renders a conditional scenario comparison panel; partner-facing `DealDetail.jsx` shows the recommended scenario badge + read-only tabs. **Internal quote dashboard (FPRM-287 / AD-23)**: new `GET /internal/quotes` with filters (status / partner / plan / search), pagination, and a system-wide summary (counts + pipeline value, expired excluded). `InternalQuotes.jsx` at `/internal/quotes` ships with summary cards + filter row + paginated table. Shared `frontend/src/utils/currency.js` `formatCurrency(amount, code)` consolidates symbol formatting across QuoteForm, QuoteDetail, DealDetail, and the new dashboard. **Partner quote history (FPRM-291)**: new `GET /partners/{id}/quotes` (partner own-org only; internal users blocked) backs `PortalQuotes.jsx` at `/portal/quotes`. `InternalDealDetail.jsx` gains a `DealHeaderQuoteBadge` in the page header that surfaces the most relevant quote (accepted > sent > draft > expired) with status + version + grand total. No new Alembic migrations in Sprint 18 — head remains **026**. Sprint 18 fix version 10769 + native sprint 742 closed. Two new ADs documented: **AD-23** (multi-currency display) + **AD-24** (quote scenario selection). **~556 backend tests passing** (Sprint 17 baseline 535 + 10 scenarios + 6 internal quotes + 5 partner quotes). Total API surface ~118 endpoints (Sprint 17 baseline 114 + 4 new) + 7 endpoints extended with `?export=csv`. Phase 5 epic FPRM-238 — Done. **Sprint 17 line that previously stood here:** Sprint 17 closed the two Phase 4 enforcement gaps that had been deferred since Sprint 13. **Dynamic activation enforcement (FPRM-270)**: `backend/activation.py` `recalculate_activation` now reads required criteria from `activation_checklist_config` (migration 022) at runtime, with a fallback to the hardcoded four-flag rule when no config rows match the partner's category/tier — preserving legacy behaviour for every existing production partner. New endpoint `GET /partners/{id}/activation/criteria` returns the resolved required criteria + per-item met state + `config_source` (`dynamic` or `fallback`). `ActivationChecklist.jsx` and `PartnerHome.jsx` switched to the dynamic endpoint. **Multi-step approval enforcement (FPRM-274)**: `POST /applications/{id}/approve` and `POST /internal/deals/{id}/approve` are now step-gated by `approval_workflow_steps` (migration 021) — the caller's role must match the current step's `required_role`. Intermediate-step approvals stamp an `ApprovalStepRecord` and return `approval_progress` without changing status; the final step runs the existing approval flow. When no steps are configured, single-step legacy behaviour is preserved. New `backend/approval_helpers.py` module (`get_approval_step_context`, `build_approval_progress`, `record_step_action`); new model `ApprovalStepRecord` + migration 026; `approval_progress` field added to GET responses for both workflows; `ApplicationReview.jsx` + `InternalDealDetail.jsx` render the step indicator with a role-gated Approve button. Two new ADs documented: **AD-21** (dynamic activation), **AD-22** (multi-step approval). Sprint 17 fix version 10768 + native sprint 741 closed. Alembic head advances 025 -> 026. **535 backend tests passing** (Sprint 16 baseline ~494 + 22 activation_dynamic + 18 approval_enforcement + 1 ad-hoc). Total API surface ~114 endpoints (Sprint 16 baseline 113 + 1 criteria endpoint) + 7 endpoints extended with `?export=csv`. Phase 5 epic FPRM-238 — Quoting Module & Enforcement. Sprint history in CLAUDE_HISTORY.md.
...
| Sprint IDs (native) | Sprint 1: `501`, Sprint 2: `534`, Sprint 3: `535`, Sprint 4: `536`, Sprint 5: `537`, Sprint 6: `538`, Sprint 7: `539`, Sprint 8: `572`, Sprint 9: `605`, Sprint 10: `638`, Sprint 11: `671`, Sprint 12: `704`, Sprint 13: `705`, Sprint 14: `706`, Sprint 15: `739`, Sprint 16: `740`, Sprint 17: `741`, Sprint 18: `742` |
| Sprint fix version IDs | Sprint 1: `10528`, Sprint 2: `10561`, Sprint 3: `10562`, Sprint 4: `10563`, Sprint 5: `10564`, Sprint 6: `10565`, Sprint 7: `10566`, Sprint 8: `10599`, Sprint 9: `10632`, Sprint 10: `10665`, Sprint 11: `10698`, Sprint 12: `10731`, Sprint 13: `10732`, Sprint 14: `10733`, Sprint 15: `10766`, Sprint 16: `10767`, Sprint 17: `10768`, Sprint 18: `10769` |
...
- **Implementation services pricing quote deferred to Phase 6.** Phase 5 covers software pricing only; the services side of a partner quote is out of scope for Phase 5.
