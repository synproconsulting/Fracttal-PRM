...
**Started:** May 2026
**Current state:** **Sprint 20 / Phase 6 (Sprint 2 of 4).** Sprint 20 was reframed from the original "Implementation Services Pricing Quote" theme (deferred pending business design input) into a **Deal Enhancements** sprint: full DEAL INFORMATION form (Section A additional prospect/engagement fields + Section B Current State and Needs Assessment using the SPICED framework), internal-side deal creation by channel managers, and add-on catalogue category/sort ordering. **Deal schema extension (FPRM-315, migration 027)**: 36 new columns on `deal_registrations` — Section A (engagement_date, prospect_phone, compiled_by, prospect_contact_name/position, prospect_website, industry_sector, company_size, feature_plan_preference), Section B Current Systems (current_system/old_system/inventory_stores/work_orders_prs/monitoring_system each accepting none|excel|paper|social_media|cmms|other), Section B Feature Requirements (15 need_* booleans + integration_with + languages_required free-text companions), Section B SPICED narratives (about_client, pain, impact, critical_event, decision, next_steps), and `created_on_behalf_of` consolidated from Story 3. **Deal form update (FPRM-316)**: partner portal `DealRegistrationForm.jsx` gains two new sections ("Additional prospect details" and "Current State and Needs Assessment (SPICED)"); `compiled_by` pre-populates from JWT email on new drafts; integration_with / languages_required follow-up inputs appear conditionally. Internal `InternalDealDetail.jsx` gains a collapsible `DealInformationSection` displaying all Section A + B data read-only (✅/❌/— for booleans, labelled blocks for narratives). Backend `CREATABLE_FIELDS` extended with 35 new keys; created_on_behalf_of stays server-only; new `_coerce_dates` helper parses ISO date strings for Postgres/SQLite parity. **Internal deal creation (FPRM-317)**: `POST /deal-registrations` now accepts channel_manager / channel_ops_admin / system_admin with `partner_org_id` in body; partner org must exist and be `PartnerStatus.active`; activation gate skipped on internal-create path; distinct `deal_registration.created_internal` audit action. New "+ New Deal" primary button on `DealQueue.jsx` opens a focused modal (Partner Org selector + Section A core + opening SPICED) — full form refactor for reuse deferred. Grey "Created by Channel Manager" badge on `InternalDealDetail.jsx` header when `created_on_behalf_of=True`. **Addon catalogue category and sort order (FPRM-318, migration 028)**: `category` (String, nullable) and `sort_order` (Integer, NOT NULL, default 0) columns on `addon_catalog_items`. `PATCH /internal/config/pricing/addons/{id}` accepts both; `GET` gains `?category=` filter (`__null__` matches uncategorised) and orders by `(category, sort_order, display_name)`. Pricing admin tab adds a category filter dropdown + inline-editable Category and Sort columns; create flow does POST then follow-up PATCH so admins get one-click create-with-organisation. Quote-form add-on selector groups by category with uppercase section headers; uncategorised items in a final "Other" group. Taxonomy itself is admin-maintainable data per AD-25 — migration 028 only creates the columns. Sprint 20 added **two migrations** (027 deal schema, 028 addon category/sort), bumping Alembic head from 026 to **028**. Sprint 20 fix version 10835 + native sprint 808 closed. No new ADs (Story 4 reinforces AD-25). **604 backend tests passing** (Sprint 19 baseline 579 + 7 deal schema + 6 deal API + 7 internal create + 5 addon category). Sprint 20 spec deviation: the Phase 6 doc claimed `qty_transactional_users`/`qty_limited_tech_users` already lived on `deal_registrations`, but they're on `quote_versions` — Story 2 omitted those two fields from the deal form to honour the "two migrations" constraint; user counts continue to be set at quote time. Phase 6 epic **FPRM-299** — Pricing Admin, Services Quote & Partner Enablement. Sprint history in CLAUDE_HISTORY.md.
...
| Sprint IDs (native) | Sprint 1: `501`, Sprint 2: `534`, Sprint 3: `535`, Sprint 4: `536`, Sprint 5: `537`, Sprint 6: `538`, Sprint 7: `539`, Sprint 8: `572`, Sprint 9: `605`, Sprint 10: `638`, Sprint 11: `671`, Sprint 12: `704`, Sprint 13: `705`, Sprint 14: `706`, Sprint 15: `739`, Sprint 16: `740`, Sprint 17: `741`, Sprint 18: `742`, Sprint 19: `775`, Sprint 20: `808` |
| Sprint fix version IDs | Sprint 1: `10528`, Sprint 2: `10561`, Sprint 3: `10562`, Sprint 4: `10563`, Sprint 5: `10564`, Sprint 6: `10565`, Sprint 7: `10566`, Sprint 8: `10599`, Sprint 9: `10632`, Sprint 10: `10665`, Sprint 11: `10698`, Sprint 12: `10731`, Sprint 13: `10732`, Sprint 14: `10733`, Sprint 15: `10766`, Sprint 16: `10767`, Sprint 17: `10768`, Sprint 18: `10769`, Sprint 19: `10802`, Sprint 20: `10835` |
