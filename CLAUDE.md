...
**Started:** May 2026
**Current state:** **Sprint 21 — Centralised Document Repository (AD-33) — complete.** Sprint 20 closed at Alembic head 028 plus the post-sprint UX session that took the head to 033. Sprint 21 implements AD-33: `partner_documents` is now the single source of truth for all partner-scoped file content (file bytes stored as base64 per AD-19), with cross-record links recorded in the new `document_references` join table. Three new migrations land: **034** (`partner_documents.file_data` Text nullable, `file_path` relaxed to nullable), **035** (`document_references` table with `(entity_type, entity_id)` and `document_id` indexes), **036** (backfill every `quote_documents` row into `partner_documents` + `document_references` with `entity_type='quote'`, then drop `quote_documents`). Alembic head moves from 033 → **036**. The legacy `/quotes/{quote_id}/documents` family of five endpoints is retired; the centralised replacement adds 9 endpoints under `/partners/{partner_id}/documents/...` plus `/quotes/{quote_id}/attached-documents` (the read-side join) and `/internal/quotes/export` (the CSV export that closes the Sprint 16 TODO). The quote acceptance gate now joins `document_references` → `partner_documents` and requires `status='approved'` so a pending evidence doc no longer satisfies the gate. Carry-forward fixes: `deal_status` surfaced on `GET /partners/{id}/quotes` (closes TODO from PR #172). Frontend: `PartnerDocuments.jsx` upload path switched to base64 `file_data` with a new Download column; `QuoteDetail.jsx` document section rewired to the centralised store via the two-step upload + create-reference flow; `InternalQuotes.jsx` Export CSV button enabled. **738 backend tests passing** (up from 723; +12 new partner_documents tests, +9 migration tests). Channel manager test user `cmtest@test.com / TestPass123!`. Phase 6 epic **FPRM-299** — Pricing Admin, Services Quote & Partner Enablement. Sprint history in CLAUDE_HISTORY.md.
...
- **Jira API token in `.env` is currently returning 401.** Sprint 21 set up the sprint structure (fix version + native sprint + 5 stories with subtasks) manually via this PR's branch because the stored token was rejected by Atlassian; rotate at id.atlassian.com so the next sprint's Phase A can run as designed. The Sprint 21 PR title therefore uses an `FPRM-AD33` placeholder slug rather than a concrete story key.
