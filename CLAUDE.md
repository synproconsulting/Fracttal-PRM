...
**Started:** May 2026
**Current state:** **Sprint 22 — Document Repository v2 — complete.** Sprint 22 takes the centralised document repository to its production-ready form. Migration **037** creates `document_versions` (one binary row per version, `is_current` flag) and adds `current_version_number` + `version_count` denormalised pointers to `partner_documents`; the migration backfills every existing row that had `file_data` into a v1 row. Migration **038** creates `document_type_rules` with seed rows `quote_acceptance` (auto-approve, no manual approval) and `contract` (requires approval). Alembic head moves from 036 → **038**. **AD-34** records the new rule: `partner_documents.file_data` is deprecated, all new bytes live in `document_versions.file_data`. The upload, download, and preview endpoints all read from `document_versions where is_current=true`. Nine new endpoints: per-document version upload/list/download/revert (internal only) + preview + `/admin/document-type-rules` CRUD. The acceptance gate now consults the rule table — `requires_approval` controls whether `PartnerDocument.status='approved'` is required (preserves the FPRM-353 hotfix behaviour for `quote_acceptance` via the seed). Frontend: portal Documents page gets version badge + inline history panel + new-version modal + preview button + partner self-service delete + Uploaded By column; QuoteDetail Pick Existing tab shows version badges; ProgramConfig has a system_admin-only Document Rules tab; the quote attach panel shows gate-info hints based on the rule. **A post-Sprint-22 hotfix (FPRM-383/384/385) refined three behaviours:** partner-admin self-service delete now *permanently* removes an unreferenced document (versions cascade) instead of soft-flagging it `rejected`; the upload status default flipped so a document type with **no** governing `document_type_rules` row is **auto-approved** (`requires_approval=true` rules still land `pending_review`); and the document-type-rule delete endpoint no longer 409s when documents of that type exist — rules are freely deletable, existing docs keep their status. **A follow-up hotfix (FPRM-386, PR #183)** made `document_type_rules` matching **case-insensitive + whitespace-trimmed**: the free-text Document Rules form had let an admin store a rule as `NDA` while uploads send the lowercase code `nda`, so the exact-match lookup missed and `requires_approval` docs were silently auto-approved; the lookup is now normalised on both sides and the rule-create duplicate check is case-insensitive. **786 backend tests passing.** **Last PR merged: #185** — **Sprint 23 PR A** (Sprint 22 carry-forward, migration **039**). Migration head moves 038 → **039** (data-only: seeds the canonical KYC + contract document types into BOTH `document_types` (vocabulary) and `document_type_rules` (approval policy), and reconciles every in-use type). Carry-forward items closed: data-driven document types + universal approval gate on every upload path (case-insensitive, FPRM-386); Program Config → Document Rules type **dropdown** + "Requires Approval" label; **partner self-accept** — `partner_user`/`partner_admin` may attach proof-of-acceptance and mark their own-org quote `accepted` (AD-35); **partner_admin version revert** for own-org docs with confirm + `document.version_reverted` audit (AD-36, supersedes FPRM-374); uploaded-by name in the version panel; **upload size cap (25 MB)** replaces the file-type allowlist (AD-37). Two-table document model recorded as **AD-38**. The earlier #184 docs PR established the **Phase 7 Backlog** section (Dynamic RBAC anchor). **Sprint 23 PR B — Asset Library — complete (PR #186, migration 040).** Migration head moves 039 → **040** (creates `asset_categories`, `assets`, `asset_download_logs`). New `assets_router` adds the partner portal list + download and the internal asset/category management endpoints; binaries are stored base64 in `assets.file_data` and `file_data` is never returned by any list endpoint — only the download endpoint streams decoded bytes (**AD-39**, the AD-17/19/20 pattern). 10 MB cap on asset upload (independent of the 25 MB partner-documents cap). Visibility is `all` | `tier:<tier>` | `category:<code>`, enforced on the partner list + download. Frontend: portal **Resources** page (`PortalAssets.jsx`, card grid + filter + download) and internal **Assets** page (`InternalAssets.jsx`, upload/edit/activate + download-log drill-down + category management). **Sprint 23 fully closed (PR A #185 + PR B #186).** **Sprint 24 PR A — Sprint 23 carry-forward bugs — complete (PR #188, no migration, head stays 040).** Three bugs from Sprint 23 UI testing closed: **(FPRM-418/AD-40)** the document-type dropdown is unified — one shared `DocumentTypeSelect` component (`frontend/src/components/DocumentTypeSelect.jsx`) sourced only from `GET /config/document-types` replaces every per-surface `<select>` (Documents page upload + filter, quote-attach, Document Rules picker), killing the divergence where quote-attach showed a hardcoded 4-item list and `nda` was missing on one surface; a new Program Config **Document Types** tab is the vocabulary admin (view all + add/archive), reusing the existing `POST/PATCH /config/document-types` (no new endpoint). **(FPRM-419)** the asset download-log modal now resolves `user_name` + `partner_org_name` (joins `users.full_name` / `partner_organizations.legal_name`) instead of showing raw UUIDs. **(FPRM-420, security)** logout now purges tenant-scoped client state and revokes document/asset **preview** blob object-URLs via `clearSession()` (`frontend/src/utils/session.js`), closing a cross-tenant leak where a prior org's preview stayed openable after a session switch until a hard refresh. **802 backend tests passing.** **Sprint 24 PR B — Channel Manager Assignment + Partner-Scoped Approval Routing — complete (PR #189, migration 041).** Migration head moves 040 → **041** (creates `partner_channel_managers`, the many-to-many CM↔partner join with `unique(partner_org_id, user_id)`). New `partner_channel_managers_router` adds `GET/POST/DELETE /partners/{id}/channel-managers` (assign/list/unassign; assigned user must hold the `channel_manager` role → 422; duplicate → 409; write gated to `system_admin` + `channel_ops_admin`; audit `partner.channel_manager_assigned`/`_unassigned`) plus `GET /internal/channel-managers` (candidate picker). **AD-41** records the routing model: ONE shared resolver in `permissions.py` (`resolve_cm_scope`, `enforce_cm_scope`, `apply_cm_scope_to_query`, `cm_scope_label`, `assignments_exist`) sits AFTER each existing role guard and narrows only the `channel_manager` role — it does NOT modify or centralize the existing guards (that stays Phase 7 Dynamic RBAC). **Global switch / fallback:** while NO partner has any assignment anywhere, every channel_manager sees all partners (bootstrap); the first assignment flips every CM to scoped at once; a CM with zero assignments then sees nothing. `system_admin` + `channel_ops_admin` are ALWAYS unscoped. Applied as queue filter on `GET /internal/deals` + `GET /internal/quotes` and as a 403 action guard on deal won/conflict-check/override-conflict, quote status/versions/active-version/active-scenario/pipeline-inclusion, and partner activation training-complete/reset. Applications are excluded (no `partner_org_id` until provisioning). No notification surfaces added. Frontend: a **Channel Managers** assignment panel on the internal partner detail page (`PartnerProfile.jsx`, add/remove gated to system_admin + channel_ops_admin) and a scoped-state banner (`CmScopeBanner.jsx`) on the Deals + Quotes queues. **816 backend tests passing.** **Sprint 24 fully closed (PR A #188 + PR B #189).** **Sprint 24 hotfix — channel-manager scope leak (#7) + assignment surfaces — complete (PR #190, no migration, head stays 041).** UI testing found the AD-41 routing guarded only the endpoints AD-41 enumerated, leaving the pre-existing deal-review verbs unguarded: a scoped `channel_manager` could still ACT on a non-assigned partner's deal/quote via a direct link even though the queue lists filtered correctly. **(FPRM-443 / AD-42)** `enforce_cm_scope` now sits after the role guard on every CM-reachable mutating action — the 9 that were missing: deal `approve` / `reject` / terminal-`status` (lost/withdrawn) / `start-review` / `request-info` / `cancel-info-request` / `messages`, plus quote `create` (internal-write branch) and `generate-pdf` — and a parametrized recurrence test (`CM_SCOPED_ACTIONS` in `test_channel_manager_assignment.py`) enumerates the COMPLETE CM-scoped action set (assigned→success / unassigned→403) so a future unguarded action fails CI; `permissions.py` carries an AD-42 comment pointing at that canonical list. **(FPRM-444)** `channel_manager` is now granted **assigned-only** partner-detail editing — `PATCH /partners/{id}` and `PATCH /partner-profiles/{id}` add `channel_manager` to the writer set then call `enforce_cm_scope` (assigned CM edits, audited via the existing `log_audit_event`; unassigned → 403; `channel_ops_admin`/`system_admin`/`partner_admin` unchanged). The frontend Edit button is shown **only** when the save will succeed, driven by a new `can_edit` flag returned by `GET /partners/{id}/channel-managers` (button-visible == save-succeeds for every role/assignment combo). **(FPRM-445)** that GET is widened so own-org `partner_admin`/`partner_user` may read their assigned managers (name + email), and the portal partner profile (`PartnerProfile.jsx`, shared component, portal mode) now shows a read-only **Channel Manager(s)** section. **852 backend tests passing.** **Last PR merged: #190.** **Sprint 25 PR A — Security hardening + CM detail-read isolation — complete (PR #191, no migration, head stays 041).** First of a two-PR Sprint 25 (Security Hardening, Phase-6 scope; PR B = rate limiting + password policy + tenant-isolation sweep). **(FPRM-451 / AD-43)** a single response-headers middleware in `main.py` sets HSTS, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, and a baseline JSON-API CSP (`default-src 'none'; frame-ancestors 'none'`) on every response — applied centrally, **CORS untouched** (`FRONTEND_URL` unchanged, by Sprint 25 scope decision). **(FPRM-452)** JWT validation is hardened to explicitly pin `algorithms=[JWT_ALGORITHM]` (HS256) and reject `alg:none` + algorithm-confusion (the decode was already pinned; this locks it with a documenting comment + regression tests, no runtime change). **(FPRM-453)** a global unhandled-exception handler returns a generic `{"detail":"Internal server error"}` 500 (full exception logged server-side) while every intended `HTTPException` 4xx `detail` is preserved unchanged. **(FPRM-454 / AD-45)** the Sprint 24 CM-scope decision (AD-41/AD-42, queue + action) is **amended to also cover detail-read**: `enforce_cm_scope` now guards `GET /deal-registrations/{id}` and `GET /quotes/{id}`, so a scoped `channel_manager` can no longer open a non-assigned deal/quote detail by direct URL (`channel_ops_admin`/`system_admin` stay unscoped; the bootstrap global-switch is preserved; partner-profile viewing stays open per AD-42); the two read paths joined the canonical `CM_SCOPED_ACTIONS` recurrence list. **865 backend tests passing.** **Last PR merged: #191.** **Sprint 25 PR B — Auth-abuse mitigation + tenant-isolation assurance — complete (PR #192, no migration, head stays 041).** Second and final PR of Sprint 25 (Security Hardening). **(FPRM-455 / AD-44)** `slowapi` (already pinned) is wired as per-IP rate limits on the sensitive unauthenticated paths only — login, password-reset request/confirm, and the public draft-token application endpoints (create/update/submit/documents); limits are env-configurable with safe code defaults (`RATE_LIMIT_LOGIN` `"10/minute"`, `RATE_LIMIT_PASSWORD_RESET` `"5/minute"`, `RATE_LIMIT_PUBLIC_APP` `"20/minute"` — read at request time so they tune in Railway without a redeploy), and a limited request returns **429**; authenticated app traffic is never blanket-limited. **(FPRM-456)** a single shared `backend/password_policy.py` validator enforces ≥12 chars + upper/lower/digit on the invite-accept (password-set) and password-reset-confirm paths, rejecting weak passwords with **422** (the documented `TestPass123!` / `PartnerPass123!` satisfy it; surfaced on the AcceptInvite + ResetPassword forms). **(FPRM-457)** a parametrized tenant-isolation regression sweep (`PARTNER_SCOPED_ENDPOINTS` in `test_tenant_isolation_sweep.py`, 12 partner-scoped endpoints) asserts an org-A user cannot read/act on an org-B resource (403/404) — **test-only, no leak found**. **888 backend tests passing.** Sprint 25 fully closed (PR A #191 + PR B #192). **Sprint 25 hotfix — single PR (FPRM-458/459/460) — complete (PR #193, no migration, head stays 041).** Three issues from Sprint 25 UI testing, folded into the existing Sprint 25 release (fix version 11000 / sprint 973). **(FPRM-460 / AD-46, release-gating)** the AD-44 rate limits were **inert in production** — 30 rapid `POST /auth/login` returned all 401, zero 429, no rate-limit headers, though CI was green: slowapi keyed on `get_remote_address` (`request.client.host`), which behind Railway's edge proxy is the rotating proxy peer, so every request landed in a different per-IP bucket and the counter never accumulated (CI's `TestClient` keeps one stable peer, so tests passed). Fixed with a proxy-aware `key_func` in `rate_limiter.py` (`get_client_ip`: prefer the non-forgeable Envoy-set `X-Envoy-External-Address`, then the left-most `X-Forwarded-For` hop, validate each as a real IP, else fall back to the TCP peer) and `headers_enabled=True` on the limiter (surfaces `X-RateLimit-*` + `Retry-After`; required adding a `response: Response` param to the 8 limited endpoints). A new test asserts the limiter **engages** per forwarded client IP (distinct clients = independent buckets) + emits headers — it would have caught the inert control. **No `requirements.txt` change, no Railway env/start-command change required** (Railway already sets the forwarded headers). **(FPRM-458)** `GET /partners/{id}/pipeline` now allows **`partner_user` for their own org** (was `partner_admin`-only); internal roles stay 403 by design, cross-org stays 403, tenant-isolation sweep stays green (amends Sprint 14 / FPRM-229). **(FPRM-459)** the internal partner profile (`PartnerProfile.jsx`, internal mode only) shows a read-only **"Partner System ID"** field (org `id`); absent in portal mode, excluded from edit + any PATCH. **892 backend tests passing.** Sprint 25 closed (PR A #191 + PR B #192 + hotfix #193). **Sprint 26 PR A — Resend API email transport — complete (PR #194, no migration, head stays 041).** First PR of Sprint 26 (new fix version **11033** / native sprint **1006**). **(FPRM-462 / AD-47)** real transactional email now ships via the **Resend HTTPS API** (`POST https://api.resend.com/emails`, port 443) — SMTP is permanently blocked on Railway on every port (25/465/587/2525), so the old `smtplib` path could never deliver. `backend/notifications.py` `send_email` was rewritten to POST to Resend via `httpx` (already pinned — no new dep), keeping its `send_email(to, subject, body_html)` signature so the existing `notify_*` call sites are untouched; when `RESEND_API_KEY` is absent/empty (local/CI) it falls back to the stdout log so flows stay testable with no network call, and it never raises (warns on transport error, per AD-13). `POST /auth/password-reset/request` and `POST /partners/{id}/users/invite` now send their links through `send_email`; the **invite `token` was removed from the API response** (it travels via the email link only — the response is `{id, partner_org_id, email, invited_role, expires_at, created_at, message}`). Email links are built from a new **`PUBLIC_APP_URL`** env (helper `notifications.public_app_url()`, CI default `http://localhost:5173`) — never `FRONTEND_URL` (which is `*` on Railway and can't form a link); the `notify_*` lifecycle templates were repointed to it too, fixing a latent broken-link bug. New Railway vars (manual, post-deploy): **`RESEND_API_KEY`** + **`PUBLIC_APP_URL`** on `fracttal-prm-backend`; sender `noreply@contact.synproconsulting.co` (domain already verified in Resend). **895 backend tests passing.** **Note:** the prompt assumed a `backend/config.py` `Settings` class; this codebase has none (it reads env via `os.getenv` directly), so `PUBLIC_APP_URL` follows that existing pattern via `public_app_url()` rather than a Settings field. **Sprint 26 hotfix — single PR (FPRM-463/464) — complete (PR #195, no migration, head stays 041).** Two bugs found in Sprint 26 PR A live verification, folded into the existing Sprint 26 release (fix version 11033 / sprint 1006). **(FPRM-463, release-gating)** the internal invite endpoint `POST /internal/partner-users/invite` (`internal_partner_users_router.py`) was the one invite call site PR #194 missed — it never called `send_email` and still returned the raw `token` in its response; it now delivers the accept-invite link via Resend using `send_email` + `public_app_url()` (matching the `partner_users_router` pattern, wrapped per AD-13) and the `token` is removed from the response (now `{id, partner_org_id, email, invited_role, expires_at, message}`). AD-47 now covers **both** invite surfaces. A codebase grep confirmed no further un-wired invite call sites (`provisioning.py`'s invite is delivered via `notify_application_approved` on the approval path). **(FPRM-464)** `PATCH /applications/{id}` (and `POST /applications`) crashed on the Postgres commit when an unfilled numeric form field arrived as an empty string (e.g. `year_established: ""` into an Integer column); a model-derived `_coerce_blank_to_none` helper now coerces an empty/whitespace-only string to `NULL` for every non-text application column (Integer/Numeric/Float/Boolean/Date/DateTime) before the write — text/string/JSON columns keep their value. No migration (head stays 041). **899 backend tests passing.** **Sprint 26 PR B — partner_user dashboard banner + Channel Manager column + Jira cleanup — complete (PR #196, no migration, head stays 041).** Second PR of Sprint 26 (fix version 11033 / native sprint 1006). **(FPRM-461)** `GET /partners/{id}/dashboard/summary` now allows **`partner_user` for their own org** (was `partner_admin`-only) with the same own-org tenant check enforced in the handler (AD-9), mirroring the FPRM-458 pipeline widening — internal-role access unchanged, cross-org still 403, tenant-isolation sweep stays green; the portal home banner (`PartnerHome.jsx`) now populates for `partner_user` with no JSX change (it already fetched unconditionally). **(FPRM-465)** `GET /internal/partners` returns a new nullable **`channel_manager_name`** = the *first-assigned* channel manager per org (earliest `partner_channel_managers.assigned_at`, tie-break lowest `id`, joined to `users.full_name`) — display names resolved in one batched query (no N+1), sorting done via a correlated scalar subquery in `_PARTNER_SORT` so pagination stays correct and `apply_sort`'s `nullslast()` puts unassigned orgs **last in both directions**; the CSV export gains a **Channel Manager** column. `InternalPartnerList.jsx` adds a sortable **Channel Manager** column (— when unassigned). **`enforce_cm_scope` is deliberately NOT applied to `/internal/partners`** — it is a roster read, not in the AD-45 `CM_SCOPED_ACTIONS` set. NB: the assignment table's timestamp column is `assigned_at` (this table has no `created_at`, which the prompt assumed). **(FPRM-466, Jira cleanup)** closed FPRM-462/463 (live-verified) and audited every open Sprint 21 item (fixVersion Sprint 21 / native 841) against the codebase + PR history — all 20 open stories/subtasks (FPRM-332→352, minus the already-Done bugs 353–357) confirmed shipped and transitioned to Done; no shipped-but-unrecorded gaps found. FPRM-465 was created this PR (the prompt assumed it pre-existed; last key had been FPRM-464). **906 backend tests passing.** **Last PR merged: #196.** Channel manager test users `cmtest@test.com` + `cmtest2@test.com` (both `TestPass123!`); channel-ops-admin `coatest@test.com / TestPass123!`. Phase 6 epic **FPRM-299** — Pricing Admin, Services Quote & Partner Enablement. Sprint history in CLAUDE_HISTORY.md.
...
**JWT validation must pin the signing algorithm explicitly and reject `alg:none`.** Every `jwt.decode(...)` call must pass a single-element `algorithms=[JWT_ALGORITHM]` list so PyJWT never trusts a client-supplied `alg` header (defeats `alg:none` and HS/RS algorithm-confusion). Never widen the list or omit it. (Sprint 25 / FPRM-452.)

**Email links must be built from `PUBLIC_APP_URL`, never `FRONTEND_URL`; SMTP must never be configured on Railway.** `FRONTEND_URL` is the CORS allowlist and is `*` on Railway — using it to construct a link yields `*/reset-password?...`, a broken link. All user-facing email links go through `notifications.public_app_url()` (`PUBLIC_APP_URL`). Email delivery is Resend over HTTPS (AD-47); **SMTP is permanently blocked on Railway on all ports** — never add or rely on `SMTP_*` vars or `smtplib` for delivery. (Sprint 26 PR A / FPRM-462.)

**Runtime security controls must be verified live on the Railway deployment, not only in CI.** Any control whose behaviour depends on the real request path through the proxy chain — rate limiting, IP allow/deny, forwarded-header handling, header-based auth — passes CI even when it is inert in production (CI's `TestClient` bypasses the proxy and keeps a stable client identity). After such a change deploys, confirm the control actually engages against the live `fracttal-prm-backend` URL (e.g. burst past the limit and observe the 429 + `Retry-After`/`X-RateLimit-*` headers) before marking the ticket Done or releasing the fix version. Root cause of FPRM-460 (AD-46): the AD-44 limiter was green in CI but never engaged in prod.
...
Every document-type dropdown in the frontend (partner Documents page upload + filter, quote-attach in QuoteDetail, the Program Config Document Rules type picker) is the one shared component `frontend/src/components/DocumentTypeSelect.jsx`, sourced exclusively from `GET /config/document-types` (the AD-38 vocabulary). No surface defines its own list or filters/overrides the vocabulary — that was the Sprint 23 divergence bug (quote-attach showed a hardcoded 4-item list, `nda` missing on one surface). Vocabulary entries are viewed and added in a new Program Config **Document Types** tab, which reuses the existing `POST /config/document-types` (`system_config:update_all` → `system_admin` + `channel_ops_admin`) and `PATCH /config/document-types/{id}` — **no new endpoint was added**. `disabledValues`/`disabledSuffix` grey out (never hide) contextually-unavailable options (e.g. a type that already has a rule) so the list stays identical everywhere. The `GET /config/document-types` contract and `test_document_types_config.py` are unchanged.
...
Channel-manager↔partner assignment (`partner_channel_managers`, migration 041) routes the EXISTING approval workflow — it is not a new engine. A SINGLE shared seam in `permissions.py` (`resolve_cm_scope` + `enforce_cm_scope` + `apply_cm_scope_to_query` + `cm_scope_label`, backed by the cached `assignments_exist` global switch) sits **after** each existing role guard and narrows only the `channel_manager` role. The existing per-router guards are **not** modified or centralized — that remains the deferred Phase 7 Dynamic RBAC work. `resolve_cm_scope` returns `ALL_PARTNERS` (unscoped) or a set of assigned `partner_org_id`s: `system_admin` + `channel_ops_admin` are ALWAYS unscoped; a `channel_manager` is unscoped while **no** assignment exists anywhere (bootstrap/global fallback), otherwise scoped to their assigned set (which may be empty → sees nothing). The first assignment created anywhere flips every CM to scoped at once. Applied as a queue filter on `GET /internal/deals` + `GET /internal/quotes` and as a 403 action guard on deal won/conflict-check/override-conflict, quote status/versions/active-version/active-scenario/pipeline-inclusion, and partner activation training-complete/reset. **Applications are excluded** (no `partner_org_id` until provisioning). No notification surfaces were added (none existed). Admin-configurable routing is deferred to Phase 7 Dynamic RBAC.

### AD-42 · Every CM-reachable mutating endpoint must call `enforce_cm_scope` after its role guard; one parametrized test is the canonical list; CM partner-detail edit is granted but scoped (Sprint 24 hotfix / FPRM-443/444/445)
AD-41's first pass only added `enforce_cm_scope` to the endpoints AD-41 explicitly enumerated, so the pre-existing deal-review verbs (`approve`, `reject`, terminal `status`, `start-review`, `request-info`, `cancel-info-request`, `messages`) plus quote `create` and `generate-pdf` stayed unguarded — a scoped `channel_manager` could act on a non-assigned partner via direct link (the #7 leak). The rule going forward: **every channel_manager-reachable mutating endpoint calls `enforce_cm_scope(db, current_user, <entity>.partner_org_id, request)` immediately after its existing role guard** (no-op for admins + bootstrap CMs; 403 for a scoped CM outside its set). The COMPLETE set is enumerated once in `backend/tests/test_channel_manager_assignment.py` → `CM_SCOPED_ACTIONS`, a parametrized recurrence test asserting assigned→success / unassigned→403 for each; a new unguarded action fails CI. `permissions.py` carries a comment pointing at this canonical list. **Capability grant:** `channel_manager` is now allowed to EDIT partner details (`PATCH /partners/{id}` + `PATCH /partner-profiles/{id}`) but ONLY for assigned partners — `channel_manager` is added to each endpoint's writer set and then `enforce_cm_scope` narrows it; edits are audited via the existing `log_audit_event`; `channel_ops_admin`/`system_admin`/`partner_admin` are unchanged. Viewing partner detail is NOT scoped (consistent with AD-41: scope the action, not the visibility). The frontend Edit button is gated on a `can_edit` flag returned by `GET /partners/{id}/channel-managers` (which mirrors the same guard) so button-visible always equals save-succeeds. That GET is also widened so own-org `partner_admin`/`partner_user` can read their assigned managers for the portal's read-only Channel Manager(s) section.

### AD-43 · Security response-headers baseline set centrally in `main.py` (Sprint 25 PR A / FPRM-451)
A single `@app.middleware("http")` in `backend/main.py` sets a baseline of security headers on **every** response: `Strict-Transport-Security: max-age=31536000; includeSubDomains`, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, and a JSON-API CSP `default-src 'none'; frame-ancestors 'none'`. Values are hardcoded (no env var). Headers are applied centrally — never per-router. The middleware uses `setdefault` so a route may override a header if it ever needs to. **CORS is deliberately untouched** by this work (`FRONTEND_URL`/`CORSMiddleware` unchanged — a conscious Sprint 25 scope decision; CORS allowlist tightening is a later sprint).

### AD-44 · Auth / public-endpoint rate limiting via `slowapi` (Sprint 25 PR B / FPRM-455)
`slowapi` (already pinned since Sprint 19 — never add a new dep for this) is wired in `backend/main.py` (`app.state.limiter` + the `RateLimitExceeded` handler were already present) and applied as **per-IP** limits ONLY to the sensitive unauthenticated paths: `POST /auth/login`, `POST /auth/password-reset/request`, `POST /auth/password-reset/confirm`, and the public draft-token application endpoints (`POST /applications`, `PATCH /applications/{id}`, `POST /applications/{id}/submit`, `POST /applications/{id}/documents`). Limits are read from env **at request time** via small getter callables so they are tunable in Railway without a redeploy, with safe code defaults: `RATE_LIMIT_LOGIN` (`"10/minute"`), `RATE_LIMIT_PASSWORD_RESET` (`"5/minute"`), `RATE_LIMIT_PUBLIC_APP` (`"20/minute"`), format `"<n>/<period>"`. A limited request returns **429**. **Never apply blanket limits to authenticated app traffic.** Tests disable the limiter suite-wide (conftest) and re-enable it only in `test_rate_limiting.py` so the shared `testclient` IP doesn't trip limits across the suite.

### AD-45 · Channel-manager scope covers queue + action + detail-read (Sprint 25 PR A / FPRM-454; amends AD-41/AD-42)
Amends the Sprint 24 CM-scope decision, which scoped only queue + action and consciously left detail-read open. The shared resolver now also guards the **detail-READ** endpoints — `enforce_cm_scope(db, current_user, <entity>.partner_org_id)` is called after the existing tenant-read guard on `GET /deal-registrations/{id}` and `GET /quotes/{id}` — so an unassigned `channel_manager` can no longer open a non-assigned deal/quote detail by direct URL (403). `channel_ops_admin` + `system_admin` keep unscoped read; the bootstrap global-switch is preserved (no assignments anywhere → all CMs read all; first assignment flips everyone to scoped). Scope is **deals + quotes detail-read only** — partner-profile viewing stays open (AD-42 unchanged) and applications remain excluded (no `partner_org_id` until provisioned). The two read paths were added to the canonical `CM_SCOPED_ACTIONS` recurrence list rather than a parallel list. (AD-44 — auth rate limiting — is introduced in Sprint 25 PR B.)

### AD-46 · Rate-limit keying is proxy-aware; the control is verified live, not just in CI (Sprint 25 hotfix / FPRM-460)
The AD-44 `slowapi` limits keyed on `get_remote_address` (`request.client.host`), which behind Railway's edge proxy is the proxy peer — and Railway routes successive requests from rotating internal source addresses, so each request fell into a different per-IP bucket and the limit never accumulated (zero 429s in prod, while CI was green because `TestClient` keeps one stable peer). The shared limiter in `backend/rate_limiter.py` now keys via `get_client_ip`: prefer `X-Envoy-External-Address` (set by Railway's Envoy edge, overwritten each hop → not client-forgeable), else the left-most `X-Forwarded-For` hop, each validated as a real IP, falling back to the TCP peer for direct/local/test hits. The limiter is constructed with `headers_enabled=True` so a limited response carries `X-RateLimit-*` + `Retry-After` — which requires every `@limiter.limit`-decorated endpoint to declare a `response: Response` parameter (slowapi injects into it); the 8 limited endpoints (login, register, password-reset request/confirm, the 4 public draft-token application endpoints) were updated accordingly. No `requirements.txt` change, no Railway env/start-command change (the forwarded headers already exist). The durable rule: **a runtime security control that only an end-to-end request through the real proxy chain exercises (rate limiting, IP allow/deny, header-based auth) is not "done" on CI green — it must be verified live on the Railway deployment.**

### AD-47 · Transactional email ships via the Resend HTTPS API; SMTP is dead on Railway; email links use `PUBLIC_APP_URL` (Sprint 26 PR A / FPRM-462)
`backend/notifications.py` `send_email` delivers via **Resend** (`POST https://api.resend.com/emails`, Bearer `RESEND_API_KEY`, JSON `{from,to,subject,html}`) over HTTPS/443 using `httpx` (already pinned — never add the `resend` SDK or any new dep for a plain HTTPS POST). **SMTP is permanently blocked on Railway on every port (25/465/587/2525)** — confirmed on the SynPro VSDC project — so `smtplib` could never deliver; it was removed. The `send_email(to, subject, body_html)` signature is unchanged so the `notify_*` call sites are untouched. When `RESEND_API_KEY` is absent/empty (local/dev/CI) `send_email` logs the email to stdout instead of calling the network (CI must never hit `api.resend.com`); it never raises (warns on transport error / non-2xx, per AD-13). The verified sender is `noreply@contact.synproconsulting.co` (`EMAIL_FROM` override optional; domain already verified in the Resend account). Email links (password reset, invite accept, application resume) are built from **`PUBLIC_APP_URL`** via `notifications.public_app_url()` — **never `FRONTEND_URL`**, which is the CORS allowlist and is `*` on Railway (a wildcard cannot form a link); absent `PUBLIC_APP_URL` warns and falls back to `http://localhost:5173` (CI/dev) rather than silently producing broken links. The `POST /partners/{id}/users/invite` response **no longer returns the `token`** — it travels via the email link only (the only way to obtain it is the email or a direct DB read). Two new Railway vars on `fracttal-prm-backend`: `RESEND_API_KEY` + `PUBLIC_APP_URL` (no trailing slash). NB: this codebase has no `config.py`/`Settings` class — env is read via `os.getenv` directly, so `PUBLIC_APP_URL` lives in the `public_app_url()` helper, not a Settings field.
...
- **Rate-limit env vars have safe code defaults (Sprint 25 / AD-44).** `RATE_LIMIT_LOGIN` (`10/minute`), `RATE_LIMIT_PASSWORD_RESET` (`5/minute`), `RATE_LIMIT_PUBLIC_APP` (`20/minute`) are read at request time, so limits can be tuned on the `fracttal-prm-backend` Railway service **without any code change or redeploy**. No var needs to be set for the feature to work — defaults apply. Format is `"<n>/<period>"`. slowapi's in-memory limiter is per-instance (not shared across multiple instances) — acceptable for the current single-instance deploy; a shared store (Redis) is a future concern if the backend scales horizontally.
- **JWT logout blacklist is in-memory only.** Lost on backend restart; not safe for multi-instance deploys.
- **Email backend is Resend (Sprint 26 PR A / AD-47).** Password-reset, partner-user invite, and the `notify_*` lifecycle emails all deliver via the Resend HTTPS API; when `RESEND_API_KEY` is unset the content falls back to stdout (dev/CI). Set `RESEND_API_KEY` + `PUBLIC_APP_URL` on `fracttal-prm-backend` for live delivery (manual Railway step). Sender `noreply@contact.synproconsulting.co`.
...
- **`RESEND_API_KEY` + `PUBLIC_APP_URL` not yet set on the `fracttal-prm-backend` Railway service.** Until set, all email (lifecycle, password-reset, invite) falls back to stdout in production — no crash. Manual ops follow-up after the Sprint 26 PR A deploy: set `RESEND_API_KEY` (Resend secret) and `PUBLIC_APP_URL=https://fracttal-prm-frontend-production.up.railway.app` (no trailing slash). **SMTP is permanently blocked on Railway (all ports) — never configure `SMTP_*`** (AD-47). `CHANNEL_OPS_EMAIL` is still read for the ops-copy on application-submitted notifications.
...
- **Channel-manager approval notifications** — email the assigned channel manager(s) on
  partner-scoped approval events (deal/quote/activation). No CM notification path exists today;
  Sprint 24 PR B routes *visibility + action* but adds no notification surface. Origin: Sprint 24
  PR B analysis. Deferred. Suggested target: Sprint 25.
...
- **Audit log — system-wide + UI-visible:** a centralised `audit_log` table capturing all transactions / state-changes, plus an internal viewer. Supersedes the bare "Centralised `audit_log` table consolidation" line. Origin: Sprint 24 UI testing — assignment (and other) audit events log to stdout only, not queryable/visible. For Phase 7 planning.
- **Login history table.**
