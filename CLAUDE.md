...
**Started:** May 2026
**Current state:** Sprint 15 complete — **Phase 5 (Quoting Module & Enforcement) underway**. Sprint 15 delivered the quoting module foundation: new tables `feature_plan_prices`, `volume_discount_tiers`, `addon_catalog_items` (migration 023 — seeded with 3 plans + 6 volume bands + 21 add-ons from the Fracttal Pricing and Quotation Specification) and `quotes`, `quote_versions`, `quote_line_items` (migration 024). New standalone `backend/quote_engine.py` implements `calculate_quote` — the single source of truth for pricing arithmetic (AD-18); pure module, no FastAPI dependencies, 21 unit tests covering all four worked spec examples and boundary cases. New `backend/routers/quotes_router.py` exposes 10 endpoints: `POST /deals/{deal_id}/quotes`, `GET /deals/{deal_id}/quotes`, `GET /quotes/{id}`, `POST /quotes/{id}/versions`, `PATCH /quotes/{id}/active-version`, `PATCH /quotes/{id}/status`, `GET /quotes/{id}/versions`, `DELETE /quotes/{id}/versions/{n}`, `GET /internal/config/pricing/plans`, `GET /internal/config/pricing/addons`. Role gating: partner_admin (own deal) + channel_manager/channel_ops_admin/system_admin for quote create + read; write roles only for new versions / activate / status; channel_ops_admin + system_admin only for soft-delete. Multi-currency `currency_code` baked into the schema from day one (display label only — no FX conversion in Phase 5). Scenario support (`scenario_label` / `quotes.active_scenario`) also schema-ready for Sprint 18 UI. No frontend in Sprint 15 — that lands in Sprint 16. Sprint 15 fix version 10766 + native sprint 739 closed. Alembic head advances 022 -> 024. **~474 backend tests passing** (Sprint 14 baseline 428 + 9 model + 21 engine + 18 API). Total API surface 111 endpoints (Sprint 14 baseline 101 + 10). Phase 5 epic FPRM-238 — Quoting Module & Enforcement. Sprint history in CLAUDE_HISTORY.md. Sprint 14 delivered Reporting & Analytics: new backend module `routers/reports_router.py` exposing five endpoints under `/internal/reports` — `/pipeline`, `/cycle-times`, `/conflicts`, `/partner-activity`, `/pipeline/export` (CSV). All aggregations are computed at query time from existing tables; no new migrations were required (Alembic head remains 022). Role gating: `system_admin`/`channel_ops_admin`/`channel_manager`/`sales_ops` for reads, with `finance_approver` additionally permitted on the CSV export. New frontend page `InternalReports.jsx` at `/internal/reports` — three sections (Pipeline Overview with stacked bar + donut + sortable partner table, Cycle Times with avg badge + monthly line chart + slowest-5 table, Conflict Report with rate badge + unresolved table), recharts-based, date-preset + category + tier filters, fetch+Blob CSV export. recharts added to `frontend/package.json`. `Reports` nav item in `InternalLayout` flipped from `Coming soon` to live. Partner pipeline view: new `GET /partners/{id}/pipeline` on `partners_router.py` (partner_admin own-org only) returning deals grouped by status; `DealList.jsx` upgraded with List/Pipeline view toggle, filter bar, summary strip, and read-only Kanban view (6 columns); `PartnerHome.jsx` gains a pipeline summary widget + `View Pipeline →` link. Sprint 14 fix version 10733 + native sprint 706 closed. **428 backend tests passing** (Sprint 13 baseline 403 + 18 reports + 7 pipeline). Total API surface 101 endpoints. Phase 4 epic FPRM-175 closed; ready for Phase 5. Sprint history in CLAUDE_HISTORY.md.
...
| Sprint IDs (native) | Sprint 1: `501`, Sprint 2: `534`, Sprint 3: `535`, Sprint 4: `536`, Sprint 5: `537`, Sprint 6: `538`, Sprint 7: `539`, Sprint 8: `572`, Sprint 9: `605`, Sprint 10: `638`, Sprint 11: `671`, Sprint 12: `704`, Sprint 13: `705`, Sprint 14: `706`, Sprint 15: `739` |
| Sprint fix version IDs | Sprint 1: `10528`, Sprint 2: `10561`, Sprint 3: `10562`, Sprint 4: `10563`, Sprint 5: `10564`, Sprint 6: `10565`, Sprint 7: `10566`, Sprint 8: `10599`, Sprint 9: `10632`, Sprint 10: `10665`, Sprint 11: `10698`, Sprint 12: `10731`, Sprint 13: `10732`, Sprint 14: `10733`, Sprint 15: `10766` |
...
- **Dynamic activation enforcement deferred to Phase 5.** `backend/activation.py` `recalculate_activation` still enforces the four hard-coded flags. The Sprint 13 `activation_checklist_config` table holds admin-configurable criteria but is not yet consumed by the enforcement function.
- **Multi-step approval enforcement deferred to Phase 5.** The Sprint 13 `approval_workflow_steps` table is read/write via `/internal/config/approval-steps` but not yet consumed by `applications_router` / `deal_registrations_router` — both still use single-reviewer flow.
- **`PartnerTier` enum vs `PartnerTierConfig` table.** Two parallel constructs since Sprint 13: the enum is referenced by `partner_organizations.tier`; the `partner_tiers` table holds admin-configurable tier records. Phase 5 will migrate the FK and retire the enum.
- **Multi-role users not supported.** A `User.role` is a single string. Sales-side roles (`sales_rep`, `sales_ops`) cannot also hold partner-side roles. Out of scope for Phase 4 — likely a Phase 5 redesign if multiple roles per user become a requirement.
