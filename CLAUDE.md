...
**Started:** May 2026
**Current state:** **Sprint 21 + post-Sprint-21 hotfix complete.** Sprint 21 landed AD-33 (centralised document repository) at Alembic head **036** with `partner_documents` as the single source of truth and `document_references` as the cross-record join. Post-Sprint-21 hotfix (FPRM-353 through FPRM-357) closed five production bugs from the UI testing session: the acceptance gate no longer requires `PartnerDocument.status='approved'` so documents uploaded through the quote modal (which arrive as `pending_review` by default) clear the gate (FPRM-353); the QuoteDetail document section now offers a two-path UI — Upload New (one-step upload + reference) OR Pick Existing (attach an existing partner document via reference only) (FPRM-354 / FPRM-355); the internal `/partners/{id}/documents` page is wrapped in a plain full-width `<div>` instead of the constrained `fp-page` class so it matches DealQueue / InternalQuotes layout (FPRM-356); `GET /internal/quotes` now returns `deal_status` on every row and both InternalQuotes.jsx and PortalQuotes.jsx render it as a tinted Deal Status column (FPRM-357). **740 backend tests passing** (Sprint 21: 738 → +2 new: pending-review acceptance + deal_status field). The hotfix did not require an Alembic migration. Channel manager test user `cmtest@test.com / TestPass123!`. Phase 6 epic **FPRM-299** — Pricing Admin, Services Quote & Partner Enablement. Sprint history in CLAUDE_HISTORY.md.
