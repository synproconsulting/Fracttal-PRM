...
**Started:** May 2026
**Current state:** Sprint 13 complete — Phase 4 (Admin Foundation & Reporting) in progress. Sprint 13 added the program-configuration surface (approval workflow steps, partner tier records, activation-checklist criteria) plus the missing partner-org status management endpoint. New backend module: `routers/program_config_router.py` exposing `/internal/config/approval-steps`, `/internal/config/tiers` (+ nested `/eligibility-rules`), and `/internal/config/activation-criteria` — read access to any internal role, writes gated on channel_ops_admin + system_admin, deletes on system_admin only. New models: `ApprovalWorkflowStep` (migration 021, 2 seed rows), `PartnerTierConfig` + `PartnerTierEligibilityRule` + `ActivationChecklistConfig` (migration 022, 3 tier seeds + 6 default activation criteria). The new `PartnerTierConfig` model is named with a `Config` suffix to coexist with the still-in-use `PartnerTier` enum that `partner_organizations.tier` references; the enum is retained until Phase 5. `internal_partners_router.py` gains `PATCH /internal/partners/{id}/status` (FPRM-208 bug fix) with audit event `partner_org.status_changed`; matching Suspend / Reactivate row actions on `InternalPartnerList.jsx` and a status badge + Change Status button on the internal `PartnerProfile.jsx` view. New frontend page: `ProgramConfig.jsx` at `/internal/config` — three tabs (Approval Workflow, Partner Tiers, Activation Checklist) wired to the new endpoints; the `Program Config` nav item in InternalLayout flipped from `Coming soon` to live (system_admin + channel_ops_admin only). Dynamic enforcement of these configurations (consuming `activation_checklist_config` in `activation.py` and routing through `approval_workflow_steps` in the application/deal flows) is deferred to Phase 5. Sprint 13 fix version 10732 + native sprint 705 closed. 403 backend tests passing. Phase 4 epic FPRM-175. Sprint history in CLAUDE_HISTORY.md.
...
| Sprint IDs (native) | Sprint 1: `501`, Sprint 2: `534`, Sprint 3: `535`, Sprint 4: `536`, Sprint 5: `537`, Sprint 6: `538`, Sprint 7: `539`, Sprint 8: `572`, Sprint 9: `605`, Sprint 10: `638`, Sprint 11: `671`, Sprint 12: `704`, Sprint 13: `705` |
| Sprint fix version IDs | Sprint 1: `10528`, Sprint 2: `10561`, Sprint 3: `10562`, Sprint 4: `10563`, Sprint 5: `10564`, Sprint 6: `10565`, Sprint 7: `10566`, Sprint 8: `10599`, Sprint 9: `10632`, Sprint 10: `10665`, Sprint 11: `10698`, Sprint 12: `10731`, Sprint 13: `10732` |
