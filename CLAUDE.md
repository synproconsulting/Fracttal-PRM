...
**Started:** May 2026
**Current state:** **Sprint 19 / Phase 6 (Sprint 1 of 4).** Sprint 19 made the pricing catalogue admin-maintainable — every Sprint 15 pricing row (plan prices, volume discount tiers, add-on catalogue) is now a data operation via the new `/internal/config/pricing/*` admin API instead of an Alembic migration. **Pricing catalogue admin API (FPRM-300 / AD-25)**: new `backend/routers/pricing_admin_router.py` exposes 9 write endpoints — `POST/PATCH/DELETE /internal/config/pricing/plans`, `GET/POST/PATCH/DELETE /internal/config/pricing/volume-tiers`, `POST/PATCH/DELETE /internal/config/pricing/addons`. Existing reads `GET /internal/config/pricing/plans` and `/addons` extended with `?include_inactive=true` (admin-only). Soft-delete pattern (`is_active=False`), system_admin-only DELETE, last-active-row guard on plan prices, range-overlap validation on volume tiers with gap-aware DELETE (overrideable via `?force=true`), case-insensitive unique addon_key on add-ons. **Pricing admin UI (FPRM-304)**: new "Pricing" tab in `ProgramConfig.jsx` with three sub-sections rendering inline edit + soft-delete + add forms; status badges (Active / Scheduled / Inactive) per plan price row; "View history" toggle on plans; "Show inactive" toggle on add-ons; reactivate flow; persistent warning banner ("Price changes take effect immediately for all new quotes. Existing quote versions are not affected."). **Pricing audit + effective-date scheduling (FPRM-308)**: `/admin/audit-log` extended with `?action_prefix=pricing` filter and `?export=csv` download (AD-20 pattern); `quote_engine.calculate_quote` now respects scheduled rows by filtering `effective_from <= today` (so a future-dated row is stored but inert until its date); new `PricingHistoryPanel` collapsible at the bottom of the Pricing tab groups events by date with discreet `Export CSV` button (system_admin only — `/admin/audit-log` requires `user_management:read_all`); new preview impact widget in plan-price `EditRow` showing a 5T + 5L compact preview plus an amber change-impact banner when any field differs from the original. No new Alembic migrations in Sprint 19 — head remains **026** (all pricing tables shipped in migration 023). Sprint 19 fix version 10802 + native sprint 775 closed. One new AD documented: **AD-25** (pricing catalogue admin-maintainable). **579 backend tests passing** (Sprint 18 baseline 556 + 18 pricing admin CRUD + 5 pricing audit/effective-date). Total API surface ~127 endpoints (Sprint 18 baseline 118 + 9 new write endpoints) + 8 endpoints with `?export=csv` (Sprint 18 baseline 7 + `/admin/audit-log`). Phase 6 epic **FPRM-299** — Pricing Admin, Services Quote & Partner Enablement. Sprint history in CLAUDE_HISTORY.md.
...
| Sprint IDs (native) | Sprint 1: `501`, Sprint 2: `534`, Sprint 3: `535`, Sprint 4: `536`, Sprint 5: `537`, Sprint 6: `538`, Sprint 7: `539`, Sprint 8: `572`, Sprint 9: `605`, Sprint 10: `638`, Sprint 11: `671`, Sprint 12: `704`, Sprint 13: `705`, Sprint 14: `706`, Sprint 15: `739`, Sprint 16: `740`, Sprint 17: `741`, Sprint 18: `742`, Sprint 19: `775` |
