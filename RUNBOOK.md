...
*Sprint 24 hotfix (2026-06-02, PR #190): channel-manager scope leak (#7) + assignment surfaces — `enforce_cm_scope` added to the 9 CM-reachable action endpoints AD-41 missed (deal approve/reject/terminal-status/start-review/request-info/cancel-info-request/messages, quote create + generate-pdf); a parametrized recurrence test (`CM_SCOPED_ACTIONS` in `test_channel_manager_assignment.py`) is now the **gate for any new CM-scoped action** — add a new CM mutating endpoint there or CI fails (AD-42). channel_manager granted **assigned-only** partner-detail edit (`PATCH /partners/{id}` + `/partner-profiles/{id}`), audited; Edit button driven by a `can_edit` flag so button-visible == save-succeeds. Partners can now **view (not edit)** their assigned channel managers on the portal profile (`GET /partners/{id}/channel-managers` widened to own-org partner roles). No migration (head stays 041). +36 tests (816 → 852).*

**Operational notes (Sprint 24 hotfix):**
- **`CM_SCOPED_ACTIONS` is the canonical list of CM-scoped mutating endpoints.** Whenever you add a channel_manager-reachable action, append it to that parametrized test (assigned→success / unassigned→403) — it is the recurrence gate that stops the #7 leak class from returning. `permissions.py` carries a comment pointing at it.
